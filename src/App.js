...
import { GET_DATA, GETDATA_BY_ID_INPUT, UPDATE_DATA, DELETE_DATA, INSERT_DATA, GET_DATA_SUBS } from 'queries/Todolist';

function TodoList() {
  // const { data, loading, error, refetch } = useQuery(GET_DATA);
  const { data, loading, error} = useSubscription(GET_DATA_SUBS);
  const [ getlazydata, {data: lazydata, loading: lazyloading, error: lazyerror}] = useLazyQuery(GET_DATA);
  const [ getDataById, { data: databyid, loading: loadingbyid, error: errorbyid }] = useLazyQuery(GETDATA_BY_ID_INPUT);
  // console.log("data=", data)
  const [ updateTodo, {data: updateData, loading: updateloading, error: updateError}] = useMutation(UPDATE_DATA, {
    onCompleted: (updateData) => {
      console.log(updateData);
      // refetch();
    },
    onError: (updateError) => {
      console.log("terjadi eror", updateError);
    }
  });

  const [ deleteTodo, {data: deleteData, loading: deleteLoading, error: deleteError}] = useMutation(DELETE_DATA, {
    onCompleted: (deleteData) => {
      console.log(deleteData);
      // refetch();
    },
    onError: (deleteError) => {
      console.log('terjadi error', deleteError);
    }
  });

  const [ insertTodo, {data: insertData, loading: insertLoading, error: insertError}] = useMutation(INSERT_DATA, {
    onCompleted: (insertData) => {
      console.log(insertData);
      // refetch();
    },
    onError: (insertError) => {
      console.log('terjadi error', insertError);
    }
  })
      ...
      <div className='container'>
            ...
            (<ul className='todo-list js-todo-list'>
              {data?.todolist.map((v, i) => (
                <Todo
                  key={i}
                  id={v.id}
                  onClickItem={() => onClickItem(v.id, v.is_done)}
        ...
        <form className='js-form' onSubmit={onSubmitList}>
          <input
            ...
            placeholder='E.g. Build a web app, then hit ENTER'
      ...
      <div className="container">
        ...
        <form className='js-form' onSubmit={(e) => {handleIdInput(e, input)}} style={{marginBottom: '0.75rem'}}>
          <input
            ...
            placeholder='Enter todolist ID, then hit ENTER'
            className='js-todo-input'
          />
        </form>
        {
            ...
            <ul className='todo-list js-todo-list'>
              { databyid?.todolist_by_pk && (
                <Todo
                  ...
                  onClickItem={() => onClickItem(databyid?.todolist_by_pk.id, databyid?.todolist_by_pk.is_done)}
